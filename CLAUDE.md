```Markdown
我正在构建一个终端 AI 编程助手（类似 Claude Code），项目名叫 HorizonCode，使用的编程语言是
[python]。

# 项目规矩
- 所有需要安装的包都通过 uv 来管理（`uv add`、`uv remove`、`uv sync`、`uv run`），不使用 pip
- 代码注释使用中文。关键类（所有公开类）和关键函数（所有公开函数、复杂的内部函数）必须有注释说明用途和参数
